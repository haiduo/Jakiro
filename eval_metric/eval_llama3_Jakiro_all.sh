...
for bench_name in "${bench_names[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES=${GPU_DEVICES} python -m jakiro.evaluation.gen_ea_answer_llama3chat \
...
for bench_name in "${bench_names[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES=${GPU_DEVICES} python -m jakiro.evaluation.gen_baseline_answer_llama3chat \
    --ea_model_path "$ea_model_path" \
    --base_model_path "$base_model_path" \
