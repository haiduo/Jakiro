...
### promopt demo: Introduce artificial intelligence to me. ###
parser = argparse.ArgumentParser()
parser.add_argument("--ea_model_path", type=str, default="/home/haiduo/model/eagle/Jakiro_vicuna-7b", help="The path to the weights.")
parser.add_argument("--base_model_path", type=str, default="/home/haiduo/model/vicuna/vicuna-7b-v1.3", help="path of basemodel")
parser.add_argument("--load_in_8bit", action="store_true", help="Use 8-bit quantization")
parser.add_argument("--load_in_4bit", action="store_true", help="Use 4-bit quantization")
parser.add_argument("--model_type", type=str, default="vicuna",choices=["llama-2-chat","vicuna","mixtral","llama-3-instruct"])
parser.add_argument("--total_token", type=int, default=60, help="The maximum number of new generated tokens.",)
...
model = EaModel.from_pretrained(
    ...
    device_map="cuda", # Change "auto" to "cuda"
)
model.eval()
warmup(model)
...
with gr.Blocks(css=custom_css) as demo:
    ...
    # 创建界面
    with gr.Row():
        gr.Image("/home/haiduo/code/Jakiro/figs/jakiro.webp", height=100, show_label=False)
        gr.Markdown("## Jakiro Chatbot")
    with gr.Row():
    ...
    with gr.Row():
        with gr.Column():
            use_EaInfer = gr.Checkbox(label="Use Jakiro", value=True)
            highlight_EaInfer = gr.Checkbox(label="Highlight the tokens generated by Jakiro", value=True)
    ...
    note=gr.Markdown(show_label=False, value='''Compression Ratio is the number of generated tokens divided by the forward passes in the original LLM. When "Highlight the tokens generated by Jakiro" is checked, correctly guessed tokens will be displayed in orange. Note: This may cause rendering issues, particularly with code generation.''')
