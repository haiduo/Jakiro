...
def evaluate_posterior(logits, candidates, logits_processor, cart_candidates_prob, alpha, alpha_num, op, p_indices, tree_candidates, b_indices):
...
def evaluate_posterior_orig(logits, candidates, logits_processor, cart_candidates_prob, alpha, alpha_num, op, p_indices, tree_candidates, b_indices):
    # Greedy decoding based on temperature value
    if logits_processor is None:
        # Find the tokens that match the maximum logits for each position in the sequence
        posterior_mask = (candidates[:, 1:].to(logits.device) == torch.argmax(logits[:, :-1], dim=-1)).int() # T=0
        candidates_accept_length = (torch.cumprod(posterior_mask, dim=1)).sum(dim=1)
        accept_length = candidates_accept_length.max()
        # Choose the best candidate
        if accept_length == 0:
