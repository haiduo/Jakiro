...
parser.add_argument('--configpath', type=str, default="/home/haiduo/code/Jakiro/jakiro_star/train/vicuna_7B_config.json") # draft model path
...
from jakiro_star.model.cnets import Model
from jakiro_star.model.configs import EConfig
...
if accelerator.is_main_process and args.enable_wandb:
    import wandb
    output_dir = "/home/haiduo/code/Jakiro/jakiro_star/outputs/wandb"
    os.makedirs(output_dir, exist_ok=True)
    wandb.init(project="Jakiro-star_vicuna-7b_e2k2_moe", entity="haiduo", config=train_config, dir=output_dir)
...
for epoch in range(num_epochs + 1):
    ...
    for batch_idx, data in enumerate(tqdm(train_loader)):
        with accelerator.accumulate(model):
            ...
            with torch.no_grad():
                # Because the head is shared by the target model and the draft model, it generates the distribution of the real target
    ...
    if (epoch + 1) % train_config["save_freq"]:
        ...
        for batch_idx, data in enumerate(tqdm(test_loader)):
            with torch.no_grad():
                ...
                predict = model(data["hidden_states"], input_ids=data["input_ids"], attention_mask=data["attention_mask"])
                # predict = predict[0] # 添加 修改 仅仅对最大expert验证
                target_head = head(data["target"])
                target_p = nn.Softmax(dim=2)(target_head)
                target_p = target_p.detach()
                loss_mask = data["loss_mask"][:, :, None]
